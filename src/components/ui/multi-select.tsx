      ...
      {[...selectedValues]
        ...
        .map((value) => (
          <Badge
            variant="outline"
            data-selected-item
            className="group flex items-center gap-1 hover:bg-secondary hover:text-secondary-foreground"
            key={value}
            onClick={
              clickToRemove
                ? (e) => {
          ...
          >
            {items.get(value)}
            {clickToRemove && (
              <XIcon className="size-2 text-muted-foreground group-hover:text-secondary-foreground transition-colors" />
            )}
          </Badge>
        ))}
      <Badge
